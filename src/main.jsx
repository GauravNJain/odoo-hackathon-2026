...
function Admin() {
  ...
  if (!data) return <Loader />;
  return <motion.section {...page} className="panel admin-panel"><Header title="Admin Panel" subtitle="Manage staff privileges, users, trips, invoices, community moderation, destinations, settings, reports, and audit logs." /><div className="stat-grid"><Stat label="Total users" value={data.users} /><Stat label="Active trips" value={data.activeTrips} /><Stat label="Revenue" value={`$${Number(data.revenue || 0).toFixed(0)}`} /><Stat label="Invoices" value={data.invoices} /></div><div className="admin-layout"><div><Header title="Manage Users" subtitle="Block/unblock users, grant roles, and inspect privileges." /><div className="staff-table"><div className="staff-row staff-head"><span>User</span><span>Role</span><span>Status</span><span>Privileges</span><span>Trips</span><span>Action</span></div>{staff.map((member) => <div className="staff-row" key={member.id}><span><strong>{member.name}</strong><small>{member.email}</small><label className="block-toggle"><input type="checkbox" checked={member.blocked} onChange={(e) => updateStaff(member, { blocked: e.target.checked })} />Blocked</label></span><select value={member.role || 'traveler'} onChange={(e) => updateStaff(member, { role: e.target.value })}><option value="admin">admin</option><option value="staff">staff</option><option value="traveler">traveler</option></select><select value={member.staff_status || 'active'} onChange={(e) => updateStaff(member, { staff_status: e.target.value })}><option value="active">active</option><option value="suspended">suspended</option></select><textarea value={member.privilege_notes || ''} onChange={(e) => updateStaff(member, { privilege_notes: e.target.value })} /><span>{member.trip_count}</span><button className="icon-btn delete" onClick={() => deleteUser(member.id)} title="Delete user"><Trash2 size={16} /></button></div>)}</div></div><div><Header title="Popular cities" /><div className="chart tall"><ResponsiveContainer><BarChart data={data.topCities}><XAxis dataKey="city" /><YAxis /><Tooltip /><Bar dataKey="count" fill="#d69332" radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer></div><Header title="Audit logs" /><div className="audit-list">{data.auditLogs.map((log) => <article key={log.id}><strong>{log.action}</strong><span>{log.actor_name || 'System'} · {log.entity_type} #{log.entity_id}</span><small>{log.created_at}</small></article>)}</div></div></div></motion.section>;
}
